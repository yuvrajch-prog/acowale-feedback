...
export const Header: React.FC<HeaderProps> = ({ activeTab, setActiveTab, adminName, onLogout }) => {
  return (
            ...
            {!adminName ? (
              ...
              <div className="flex items-center gap-3">
                {activeTab === 'user' ? (
                  ...
                  <button
                    onClick={() => setActiveTab('user')}
                    className="px-3.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 font-semibold text-xs flex items-center gap-1.5 transition shadow-xs cursor-pointer"
                  >
                    <MessageSquarePlus className="w-3.5 h-3.5 text-slate-500" />
                    View Feedback Form
                  </button>
